#!/bin/bash

echo "=== API兼容性分析示例 ==="
echo

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$SCRIPT_DIR"
LIB_PARSE="$ROOT_DIR/src/lib_parse.py"
ANALYZER="$ROOT_DIR/src/api_compatibility_analyzer.py"

# 创建输出目录
if [ ! -d "$ROOT_DIR/output" ]; then
    mkdir -p "$ROOT_DIR/output"
fi

echo "1. 解析测试库 v1.0"
echo "========================================"

python "$LIB_PARSE" \
    --root_path "$ROOT_DIR/test_lib/v1.0" \
    --output_path "$ROOT_DIR/output/api_v1.json" \
    --verbose

echo
echo "2. 解析测试库 v2.0"
echo "========================================"

python "$LIB_PARSE" \
    --root_path "$ROOT_DIR/test_lib/v2.0" \
    --output_path "$ROOT_DIR/output/api_v2.json" \
    --verbose

echo
echo "3. 生成JSON格式的详细报告"
echo "========================================"

# 生成JSON格式报告
python "$ANALYZER" \
    "$ROOT_DIR/output/api_v1.json" \
    "$ROOT_DIR/output/api_v2.json" \
    --output "$ROOT_DIR/output/compatibility_report.json" \
    --format json

echo
echo "=== 分析完成 ==="
echo "生成的文件："
echo "  - API数据: $ROOT_DIR/output/api_v1.json"
echo "  - API数据: $ROOT_DIR/output/api_v2.json" 
echo "  - JSON报告: $ROOT_DIR/output/compatibility_report.json"
echo
